...
export interface MediaLibraryCopy {
  title: string;
  /** The confirm action, e.g. "Use for this item". */
  confirmLabel: string;
  ...
  /**
   * Optional consumer note near the actions — the storefront adapters use
   * it to state the staging contract (selection claims nothing; the draft
   * save performs the claim, and nothing is public until publication).
   */
  footnote?: string;
}
...
export function MediaLibraryDialog({
  ...
  return (
    <Dialog title={copy.title} pending={pending} onCancel={onCancel}>
      ...
      {selected !== null && !decorative && (
        <fieldset className={styles.fieldset}>
          ...
          <div className={styles.check}>
            ...
            />
            <label htmlFor="alt-decorative">{copy.decorativeLabel}</label>
          </div>
        </fieldset>
      )}
      ...
      {copy.footnote !== undefined && (
        <p className={styles.fieldHintText}>{copy.footnote}</p>
      )}
      ...
      <div className={styles.actions}>
        ...
        >
          {pending ? copy.pendingConfirmLabel : copy.confirmLabel}
        </button>
      </div>
